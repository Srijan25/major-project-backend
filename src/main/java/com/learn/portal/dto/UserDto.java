...
//	@Size(max = 16, message ="Password should not exceed length of 16")
    //@Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$#!%*?&])[A-Za-z\\d@$#!%*?&]+$", message = "Password should contain at least one upper-case letter, lower-case letter, a digit or special character")
    //@Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[^\\w])[A-Za-z\\d\\W]+$", message = "Incorrect Password Format")
